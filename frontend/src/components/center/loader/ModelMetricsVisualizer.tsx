...
interface ModelMetricsVisualizerProps {
  data: MetricPoint[];
  metricKind: 'accuracy' | 'f1' | 'rmse';
}

const metricLabelMap: Record<ModelMetricsVisualizerProps['metricKind'], string> = {
  accuracy: 'Accuracy',
  f1: 'Macro F1',
  rmse: 'RMSE',
};

export default function ModelMetricsVisualizer({ data, metricKind }: ModelMetricsVisualizerProps) {
  const currentTrain = data.length > 0 ? data[data.length - 1]?.trainMetric : null;
  const currentVal = data.length > 0 ? data[data.length - 1]?.valMetric : null;
  const currentEpoch = data.length > 0 ? data[data.length - 1]?.epoch : 0;

  const label = metricLabelMap[metricKind];
  const domain = metricKind === 'rmse' ? ([0, 'auto'] as const) : ([0, 1] as const);
    ...
    <div className="w-full">
      <div className="rounded-2xl border border-replit-border/60 bg-replit-surface/35 backdrop-blur p-6 pb-5">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <div className="text-lg font-semibold text-replit-text">Model Metric</div>
          </div>
        ...
        <div className="bg-replit-bg/30 rounded-xl p-4 mb-4 relative overflow-hidden border border-replit-border/60">
          ...
          <ResponsiveContainer width="100%" height={620}>
            <LineChart data={data} margin={{ top: 5, right: 24, left: 14, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(148,163,184,0.25)" />
              <XAxis
                dataKey="epoch"
                stroke="rgb(var(--replit-text-muted))"
                label={{ value: 'Epoch', position: 'insideBottom', offset: -5, fill: 'rgb(var(--replit-text-muted))' }}
              />
              <YAxis
                stroke="rgb(var(--replit-text-muted))"
                label={{ value: label, angle: -90, position: 'insideLeft', fill: 'rgb(var(--replit-text-muted))' }}
                domain={domain as unknown as (string[] | number[])}
              />
              <Tooltip
                contentStyle={{
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-replit-accent/10 border border-replit-accent/30 rounded-lg p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-replit-accent font-semibold">Training {label}</span>
            </div>
            <p className="text-replit-text text-2xl font-mono">{currentTrain !== null ? currentTrain.toFixed(4) : '—'}</p>
          </div>
          <div className="bg-replit-warning/10 border border-replit-warning/30 rounded-lg p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-replit-warning font-semibold">Validation {label}</span>
            </div>
            <p className="text-replit-text text-2xl font-mono">{currentVal !== null ? currentVal.toFixed(4) : '—'}</p>
          </div>
        </div>
      </div>
    </div>
