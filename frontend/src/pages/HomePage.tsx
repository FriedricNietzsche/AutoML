...
export default function HomePage() {
  ...
  const onStart = async () => {
    ...
    // Call backend to start demo workflow
    try {
      const apiBase = (typeof window !== 'undefined' && (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1'))
        ? 'http://localhost:8000'
        : '';

      await fetch(`${apiBase}/api/demo/run/${session.id}?prompt=${encodeURIComponent(message)}`, {
        method: 'POST',
      });
    } catch (err) {
      console.error('[HomePage] Failed to trigger demo workflow:', err);
    }
  ...
  return (
    <div
      className={`min-h-screen overflow-hidden relative flex flex-col transition-colors duration-500 ${isDarkMode ? 'bg-slate-950 text-white' : 'bg-gray-50 text-gray-900'
        }`}
    >
      ...
      {/* Floating orbs */}
      <div
        className={`fixed top-20 left-20 w-64 h-64 bg-blue-500 rounded-full mix-blend-multiply filter blur-3xl animate-pulse ${isDarkMode ? 'opacity-20' : 'opacity-10'
          }`}
      />
      <div
        className={`fixed top-40 right-20 w-96 h-96 bg-sky-500 rounded-full mix-blend-multiply filter blur-3xl animate-pulse ${isDarkMode ? 'opacity-20' : 'opacity-10'
          }`}
        style={{ animationDelay: '1s' }}
      />
      <div
        className={`fixed bottom-20 left-1/2 w-80 h-80 bg-indigo-500 rounded-full mix-blend-multiply filter blur-3xl animate-pulse ${isDarkMode ? 'opacity-20' : 'opacity-10'
          }`}
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <button
            ...
            className={`p-3 rounded-xl transition-all duration-300 transform hover:scale-110 ${isDarkMode
              ...
              } backdrop-blur-sm border ${isDarkMode ? 'border-blue-500/30' : 'border-blue-300/50'
              }`}
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center">
        <div className="mx-auto w-full max-w-7xl px-6 py-12">
          ...
          >
            <div
              className={`inline-block mb-6 px-4 py-2 rounded-full text-sm font-medium backdrop-blur-sm animate-pulse ${isDarkMode
            ...
            <p
              className={`text-xl md:text-2xl max-w-3xl mx-auto transition-colors duration-500 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'
                }`}
            >
              Ready to dive in?
            </p>
          ...
          <div className="mt-10 w-full max-w-4xl mx-auto">
            <motion.div
              ...
              className={`relative backdrop-blur-xl rounded-3xl border shadow-2xl p-2 transition-all duration-300 ${isDarkMode
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="relative">
                  <button
                    ...
                    className={`p-3 rounded-xl transition-all duration-300 transform hover:scale-110 group ${isDarkMode ? 'hover:bg-slate-700/50' : 'hover:bg-gray-100'
                      }`}
                    aria-label="Open attachments"
                  >
                    <Plus
                      className={`w-6 h-6 transition-all duration-300 ${isDropdownOpen ? 'rotate-45' : ''
                        } ${isDarkMode
                  ...
                  {isDropdownOpen && (
                    <div
                      className={`absolute bottom-full left-0 mb-2 w-80 backdrop-blur-xl border rounded-2xl shadow-2xl overflow-hidden ${isDarkMode
                      ...
                      {dropdownOptions.map((option) => (
                        <div key={option.label}>
                          <button
                            ...
                            className={`w-full flex items-center gap-3 px-4 py-3 transition-all duration-200 group ${isDarkMode
                          ...
                          >
                            ...
                            <span
                              className={`transition-colors flex-1 text-left ${isDarkMode
                          ...
                          {option.kind === 'dataset' && (
                            <div className="px-4 pb-4">
                              <input
                                ...
                                className={`w-full text-sm px-3 py-2 rounded-xl border outline-none bg-transparent transition ${isDarkMode
                              ...
                              />
                              <div
                                className={`mt-2 text-xs ${isDarkMode ? 'text-gray-500' : 'text-gray-500'
                                  }`}
                              >
                ...
                <textarea
                  ...
                  className={`flex-1 bg-transparent outline-none resize-none py-2.5 px-2 leading-6 max-h-32 min-h-[3rem] transition-colors duration-500 ${isDarkMode
                ...
                <button
                  onClick={onStart}
                  disabled={!canStart}
                  className={`p-3 rounded-xl transition-all duration-300 transform ${canStart
            ...
            <p
              className={`text-center text-sm mt-4 transition-colors duration-500 ${isDarkMode ? 'text-gray-500' : 'text-gray-400'
                }`}
            >
